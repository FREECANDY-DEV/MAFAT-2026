...
# Start checking from 2 hours ago to ensure we catch CloudTrail delivery delays
last_hours = today - timedelta(hours=2)
start_after = f"{prefix}{last_hours.strftime('%Y-%m-%d-%H')}"
print(f"Checking logs starting after: {start_after}")
...
if 'Contents' in response:
    for obj in response['Contents']:
        ...
        try:
            ...
            if ua and "MARKER" in ua:
                print(f"Found our marker! File: {obj['Key']}")
                print(f"UserAgent: {ua}")
                print(f"ErrorCode: {log_data.get('errorCode')}")
                print("---")
                found = True
...
if not found:
    print("Marker not found in recent logs.")
