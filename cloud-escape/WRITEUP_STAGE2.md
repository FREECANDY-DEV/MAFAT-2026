...
### <samp>STEP 08</samp> ✦ Data Exfiltration Constraints & AWS WAF
During the attempts to actively extract data (such as environment variables or `flag.txt`) from the Lambda execution context, multiple exfiltration techniques were met with robust environmental defenses:

1. **CloudTrail Header Injection (Log Delay):**
   - We attempted to inject flag data directly into the `User-Agent` HTTP header of S3 API calls generated from within the Lambda (e.g., `User-Agent: FLAG_{content}`).
   - While the calls executed, the S3 Server Access Logs / CloudTrail delivery mechanism proved too slow (often taking several minutes to deliver log files to `logd8a2f72fe43094e8`), making iterative extraction impractical.

2. **Timing & Error-Based Oracles:**
   - We developed a boolean timing oracle (injecting `time.sleep()` for correct character guesses) and an exception-based oracle (raising custom exceptions).
   - The Lambda environment features a strict ~15-second timeout, and API Gateway suppresses custom exception messages, returning a static `{"error": "Something went wrong!"}`.
   
3. **AWS WAF Rate Limiting (Bot Control):**
   - Automated scripts performing binary searches across the character set triggered **HTTP 403 Forbidden** responses from AWS WAF.
   - We implemented exponential backoff, randomized jitter, and dynamically rotating `User-Agent` strings in our local scripts to bypass the WAF rate limits and continue the extraction slowly but steadily.
...
<table>
  ...
  <tbody>
    ...
    <tr>
      <td><code>WAF Bypassing</code></td>
      <td><img src="https://img.shields.io/badge/In%20Progress-F79211?style=flat-square" alt="In Progress" /></td>
      <td>Implemented jitter and backoff to bypass 403 Forbidden rate-limiting on Oracle requests.</td>
    </tr>
  </tbody>
</table>
