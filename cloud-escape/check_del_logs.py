...
prefix = "userd8a2f72fe43094e8/DeleteObject/"
...
if 'Contents' in response:
    for obj in response['Contents']:
        ...
        try:
            ...
            print(f"UserIdentity: {detail.get('userIdentity', {}).get('arn')}")
            print(f"UserAgent: {detail.get('userAgent')}")
            print(f"ErrorCode: {detail.get('errorCode')}")
            print(f"RequestParameters: {detail.get('requestParameters')}")
            print(f"ResponseElements: {detail.get('responseElements')}")
            print("---")
        except json.JSONDecodeError:
            pass
else:
    print("No DeleteObject logs found.")
