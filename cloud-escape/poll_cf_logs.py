import os
import json
import boto3
import time
from datetime import datetime, timezone

session = boto3.Session(
    aws_access_key_id="ASIARYWSMSYIMLIFTXQE",
    aws_secret_access_key="2pFYytUGEJ61BMm0LEWESi0d71uWsf2TjoGpwObR",
    aws_session_token="",
    region_name="us-east-1"
)
s3 = session.client('s3')
log_bucket = "logd8a2f72fe43094e8"
prefix = "userd8a2f72fe43094e8/GetObject/2026-08-05"

print("Polling for CloudFront failed requests...")

found_uas = set()
checked_keys = set()
for _ in range(30):
    paginator = s3.get_paginator('list_objects_v2')
    pages = paginator.paginate(Bucket=log_bucket, Prefix=prefix)
    
    for page in pages:
        if 'Contents' in page:
            # Sort by LastModified
            for obj in sorted(page['Contents'], key=lambda x: x['LastModified'], reverse=True):
                if obj['Key'] in checked_keys:
                    continue
                checked_keys.add(obj['Key'])
                try:
                    log_obj = s3.get_object(Bucket=log_bucket, Key=obj['Key'])
                    log_content = log_obj['Body'].read().decode('utf-8')
                    log_data = json.loads(log_content)
                    detail = log_data.get('detail', {})
                    key = detail.get('requestParameters', {}).get('key', '')
                    error_code = detail.get('errorCode', '')
                    user_agent = detail.get('userAgent', '')
                    
                    if key == 'flag.txt':
                        if user_agent and not user_agent.startswith('Boto3') and not user_agent.startswith('MARKER-CODE-'):
                            if user_agent not in found_uas:
                                print(f"[{error_code}] Key: {key} -> UserAgent: {user_agent}")
                                found_uas.add(user_agent)
                except Exception:
                    pass
    
    if len(found_uas) > 0 and "MARKER+TEST+UA" not in list(found_uas)[-1]:
        print("Found a new User-Agent!")
        break
    time.sleep(20)
print("Done polling.")
