...
def get_ex():
    print("Extracting lambda_function.py fast...", flush=True)
    length = 2000
    res = [""] * length
    
    # Let's resume from position 140
    for chunk_start in range(140, length, 50):
        chunk_end = min(length, chunk_start + 50)
        threads = []
        queues = []
