...
def test_string_presence(substring):
    code = f"""
import time
if '{substring}' in list(globals().keys()):
    time.sleep(2.5)
"""
...
for sub in ["event", "context", "handler", "aws", "urllib"]:
    print(f"Contains '{sub}': {test_string_presence(sub)[1]}")
