import base64
import requests
import time
from aws_requests_auth.aws_auth import AWSRequestsAuth

auth = AWSRequestsAuth(aws_access_key="ASIARYWSMSYIMLIFTXQE",
                       aws_secret_access_key="2pFYytUGEJ61BMm0LEWESi0d71uWsf2TjoGpwObR",
                       aws_token="",
                       aws_host='l8ssyaz69f.execute-api.us-east-1.amazonaws.com',
                       aws_region='us-east-1',
                       aws_service='execute-api')

url = "https://l8ssyaz69f.execute-api.us-east-1.amazonaws.com/dev/code_exec"

def test_exception(try_block, name):
    code = f"""
import time
import urllib.request
import boto3
try:
    {try_block}
    time.sleep(2.5)
except Exception as e:
    pass
"""
    encoded_code = base64.b64encode(code.encode('utf-8')).decode('utf-8')
    start = time.time()
    try:
        requests.post(url, json={"code": encoded_code}, auth=auth, timeout=6)
    except:
        pass
    elapsed = time.time() - start
    return name, elapsed > 2.0

tests = [
    ("session = boto3.Session()", "Session creation"),
    ("s3 = boto3.client('s3')", "S3 client creation"),
    ("s3 = boto3.client('s3')\\n    s3.list_buckets()", "List buckets")
]

for t, n in tests:
    print(f"{n} successful: {test_exception(t, n)[1]}")
