import os
import json
import boto3

session = boto3.Session(
    aws_access_key_id="ASIARYWSMSYINWI54QUF",
    aws_secret_access_key="vzoDaHLWP97VayPQRZpwLhu9+wucg+FdyJbsgKBQ",
    aws_session_token="",
    region_name="us-east-1"
)
s3 = session.client('s3')
log_bucket = "logd8a2f72fe43094e8"

prefix = "userd8a2f72fe43094e8/GetObject/"
print("Checking earliest GetObject logs...")

response = s3.list_objects_v2(Bucket=log_bucket, Prefix=prefix, MaxKeys=50)
if 'Contents' in response:
    for obj in response['Contents']:
        log_obj = s3.get_object(Bucket=log_bucket, Key=obj['Key'])
        log_content = log_obj['Body'].read().decode('utf-8')
        try:
            log_data = json.loads(log_content)
            detail = log_data.get('detail', {})
            error_code = detail.get('errorCode')
            if not error_code:
                key = detail.get('requestParameters', {}).get('key', 'unknown')
                user_agent = detail.get('userAgent', 'unknown')
                print(f"[Success] Date: {obj['Key']} | Key: {key} | UserAgent: {user_agent}")
        except json.JSONDecodeError:
            pass
else:
    print("No GetObject logs found.")
