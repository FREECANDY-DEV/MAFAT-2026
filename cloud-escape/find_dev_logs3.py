import boto3
import json
import concurrent.futures

session = boto3.Session(
    aws_access_key_id="ASIARYWSMSYIIWK4BEMG",
    aws_secret_access_key="yQXeKSwcKm2sMkj34DVhMhzMAQ/U+U1en5S4xmsd",
    aws_session_token="",
    region_name="us-east-1"
)
s3 = session.client('s3')
log_bucket = "logd8a2f72fe43094e8"
prefix = "userd8a2f72fe43094e8/GetObject/"

paginator = s3.get_paginator('list_objects_v2')
pages = paginator.paginate(Bucket=log_bucket, Prefix=prefix)

keys = []
for page in pages:
    if 'Contents' in page:
        for obj in page['Contents']:
            keys.append(obj['Key'])

print(f"Total logs: {len(keys)}. Reading oldest logs...")
keys = keys[:2000]

def process_key(key):
    try:
        log_obj = s3.get_object(Bucket=log_bucket, Key=key)
        log_content = log_obj['Body'].read().decode('utf-8')
        log_data = json.loads(log_content)
        detail = log_data.get('detail', {})
        req_key = detail.get('requestParameters', {}).get('key', '')
        if req_key == 'flag.txt':
            user = detail.get('userIdentity', {}).get('arn', '')
            err = detail.get('errorCode', '')
            user_agent = detail.get('userAgent', '')
            
            if 'ctf_participant_role' not in user:
                return f"NON-PARTICIPANT | Err: {err} | UA: {user_agent} | Full: {log_content}"
            if not err:
                return f"SUCCESS | UA: {user_agent} | Full: {log_content}"
    except Exception as e:
        pass
    return None

found = []
with concurrent.futures.ThreadPoolExecutor(max_workers=50) as executor:
    futures = {executor.submit(process_key, key): key for key in keys}
    for future in concurrent.futures.as_completed(futures):
        res = future.result()
        if res:
            found.append(res)
            print(res)

print("Done.")
